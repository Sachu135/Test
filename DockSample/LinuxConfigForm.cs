        ...
        void RunScript()
        {
            try
            {
                Task t = new Task(() => {
                    ...
                    diCmds.Add(@"sudo apt update;sudo apt install snapd;sudo snap install dotnet-runtime-31;sudo snap install dotnet-sdk --classic --channel=3.1;sudo snap alias dotnet-sdk.dotnet dotnet; dotnet --version;", new List<string> { "dotnet-sdk 3.1 from Microsoft .NET Core", "snap \"dotnet-sdk\" is already installed, see 'snap help refresh'", "3.1.404" });
                    diCmds.Add(@"bash /etc/KockpitStudio/Packages/Installer/updatePath.sh; sudo killall apt apt-get; sudo apt-get install supervisor -y;", new List<string> { "Processing triggers for systemd (237-3ubuntu10.42) ...", "triggers for systemd", "supervisor is already the newest version", "supervisor>" });
